...
export default function Checkout({ cartItems = [], setCartItems, clearCart }) {
  ...
  const handleQuantityChange = async (id, delta) => {
    ...
    const currentStock = Number(
      liveProduct?.stock_count ?? liveProduct?.stock ?? currentItem.stock ?? 0
    );

    const newQty = (Number(currentItem.quantity) || 1) + delta;

    // Allow decreasing/removing even if the product has gone out of stock.
    if (delta < 0) {
    ...
    // Prevent quantity from exceeding the latest stock.
  ...
  const handleSubmit = async (e) => {
    ...
    // Re-check live stock before placing the order.
    // This prevents checkout with quantities that became unavailable
    // after the cart was opened.
    const liveProducts = await getLiveProducts();
    ...
    for (const item of items) {
      ...
      if (currentStock <= 0) {
        showToast(`${item.title || item.name} is Out of Stock.`, "danger");
        return;
      }

      if (requestedQty > currentStock) {
        showToast(
          `${item.title || item.name}: Only ${currentStock} item(s) available in stock.`,
          "danger"
        );
        return;
      }
